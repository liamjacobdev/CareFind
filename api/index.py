...
InNetwork's data path (Medicare/TiC lookups) is read-only at serve time, so we ship a
...
Everything below runs at import (cold start), BEFORE app.config reads the environment:
  • seed /tmp/innetwork.db from the committed innetwork.db.gz;
...
_SEED = _ROOT / "innetwork.db.gz"
_RUNTIME = "/tmp/innetwork.db"
...
os.environ["INNETWORK_DB"] = _RUNTIME
