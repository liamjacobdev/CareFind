...
const PLANS = { plans: [MEDICARE],
                categories: [{ id: 'medicare', label: 'Medicare', plans: [MEDICARE] }] };
const SEARCH = {
  count: 1, total: 1, truncated: false, pool_capped: false, applied_filters: ['medicare'],
  context_plans: [], plans: [MEDICARE],
  providers: [{
    ...
    insurance: { medicare: { value: true, confidence: 'verified', source: 'medicare', level: 'plan',
                             source_url: 'https://data.cms.gov/...', fetched_at: 1700000000 } },
    lat: 30.77, lng: -86.58,
  }],
};
...
test('golden journey: plan → ZIP → specialty → verified provider (<30s, <5 interactions)', async ({ page }) => {
  ...
  await page.fill('#zip-input', '32536'); interactions++;
  await page.selectOption('#specialty-select', 'Cardiology'); interactions++;
  await page.click('#search-btn'); interactions++;
