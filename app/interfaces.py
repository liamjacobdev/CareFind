...
class GeocoderBackend(Protocol):
    ...
    async def geocode_one(self, q: str): ...
    async def geocode_batch(self, items: list, budget_seconds: float | None = None) -> dict: ...
    async def reverse(self, lat, lon) -> str: ...
...
class Datastore(Protocol):
    ...
    def medicare_has_many(self, npis: list) -> set: ...
    def medicare_add_many(self, npis) -> int: ...
    ...
    def tic_has_many(self, payer: str, npis: list) -> set: ...
    def tic_add_many(self, payer: str, npis) -> int: ...

    # Geocode + reverse-geocode caches
    def geocode_get(self, key: str): ...
    def geocode_set(self, key: str, lat, lon) -> None: ...
    def revgeocode_get(self, key: str): ...
    def revgeocode_set(self, key: str, postcode: str) -> None: ...

    # FHIR Plan-Net result cache
    def fhir_cache_get(self, payer: str, npi: str): ...
    def fhir_cache_get_many(self, payer: str, npis) -> dict: ...
    def fhir_cache_set(self, payer: str, npi: str, value: str, fetched_at: float) -> None: ...
    def fhir_cache_set_many(self, payer: str, items, fetched_at: float) -> int: ...

    # Source provenance (verify URL + fetch date)
    def source_meta_set(self, source_id: str, source_url: str, fetched_at: float) -> None: ...
    def source_meta_get(self, source_id: str): ...
